...
6. **Add the routing-resolution script.** Save `roiam_resolveRoutingFromPd.groovy` from section 9. Place under `scripts/collections/resilientOrderHub/`. Inside the iFlow project, under `script/v2/`. Upload via Script step dialog, then click *Upgrade* (top-right) — same rule as always. Place after the idempotency check.
